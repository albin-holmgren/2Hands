...
BEGIN
  RETURN QUERY
  WITH claimed AS (
    UPDATE integration_delivery_log
    SET status = 'processing',
        attempt_count = attempt_count + 1,
        last_attempt_at = NOW(),
        next_attempt_at = NULL,
        updated_at = NOW(),
        response = COALESCE(response, '{}'::JSONB) || jsonb_build_object('claim', jsonb_build_object('claimed_by', p_worker_id, 'claimed_at', NOW()))
    WHERE id IN (
      SELECT l.id
      FROM integration_delivery_log l
    ...
    RETURNING integration_delivery_log.*
  )
