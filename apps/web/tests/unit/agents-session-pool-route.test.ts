...
import { GET, POST } from '../../src/app/api/agents/session-pool/route'
...
function makeRequest(method: 'GET' | 'POST', headers: Record<string, string> = {}): NextRequest {
  return new NextRequest('http://localhost:3000/api/agents/session-pool', {
    method,
    headers,
  })
}

console.log('\n🧊 Session Pool Route Tests\n')

async function run() {
  const originalEnv = { ...process.env }

  try {
    // Missing CRON secret should fail closed
