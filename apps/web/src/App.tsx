...
export function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("rakazo:session-unauthorized", refresh);
    return () => window.removeEventListener("rakazo:session-unauthorized", refresh);
  }, [session.refetch]);
  ...
  const user = gate === "authenticated" ? session.data?.user : undefined;
  return (
      ...
      <Suspense fallback={<div className="h-full bg-[var(--rk-page)]" />}>
        <Routes>
          ...
          <Route
            ...
            element={
              user ? (
                <Navigate to={authReturnPath(location.state)} replace />
              ) : (
          ...
          <Route
            path="/sign-up"
            element={user ? <Navigate to="/app" replace /> : <AuthPage key="up" mode="up" />}
          />
          <Route
...
function ShellSkeleton() {
  return (
    <div
      ...
      data-rakazo-app-state="session-pending"
    >
      <aside className="hidden w-[272px] shrink-0 border-e border-[var(--rk-hairline)] bg-[var(--rk-sidebar)] px-3.5 pt-16 md:block">
        ...
        <div className="mt-5 space-y-2 px-1">
          {[0, 1, 2, 3].map((row) => (
            <div key={row} className="flex items-center gap-3 rounded-xl px-2 py-2.5">
              <div className="h-9 w-9 rounded-full bg-[var(--rk-surface-2)]" />
              <div className="flex-1 space-y-2">
                <div className="h-3 w-2/5 rounded bg-[var(--rk-surface-2)]" />
                <div className="h-2.5 w-4/5 rounded bg-[var(--rk-surface)]" />
              </div>
            </div>
          ))}
        </div>
      ...
      <main className="flex flex-1 flex-col">
        <div className="h-[60px] border-b border-[var(--rk-hairline)]" />
        <div className="flex flex-1 items-center justify-center text-[14px] text-[var(--rk-muted-2)]">
          <Trans>Opening your Space…</Trans>
        </div>
        <div className="mx-6 mb-6 h-[114px] rounded-[24px] border border-[var(--rk-hairline-strong)] bg-[var(--rk-surface)]" />
      </main>
    </div>
  );
}
