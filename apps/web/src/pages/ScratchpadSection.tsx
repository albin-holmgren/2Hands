    ...
    <div className="mt-6" data-testid="bot-scratchpad">
      ...
      {items.length === 0 ? (
        ...
        items.map((item) => (
          <div
            key={item.id}
            className="flex w-full items-start gap-2 rounded-[11px] px-2.5 py-2.5 hover:bg-[var(--rk-surface-2)]"
          >
            <button
              type="button"
              ...
              className="mt-0.5 h-4 w-4 shrink-0 rounded border border-[var(--rk-hairline-strong)] text-[10px] leading-none text-[#E65707]"
            >
              {item.status === "done" ? "✓" : ""}
            </button>
      ...
      >
        <input
          ...
          maxLength={200}
          className="min-w-0 flex-1 rounded-[11px] border border-[var(--rk-hairline-strong)] bg-transparent px-3 py-2 text-[14px] text-[var(--rk-ink)] placeholder:text-[var(--rk-muted-2)]"
        />
