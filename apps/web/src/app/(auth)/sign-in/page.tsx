...
function AuthForm() {
  ...
  return (
    <div className="w-full max-w-[400px] mx-auto text-center">
      ...
      {/* Title - match Sana: text-3xl, subtitle */}
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        ...
        className="mb-6"
      >
        <h1 className="text-3xl font-bold text-foreground leading-[1.3] mb-3">
          Welcome back
        </h1>
        <p className="text-[15px] text-muted-foreground">
          ...
          {step === 'signin' && 'Welcome back! Enter your password to sign in'}
      ...
      <AnimatePresence mode="wait">
        {step === 'email' && (
          ...
          >
            ...
            {/* Email Input - Sana style: min-h-12, rounded-3xl, font-bold text */}
            <form onSubmit={handleEmailSubmit} className="space-y-3">
              <input
                ...
                autoComplete="email"
                autoFocus
                className="w-full min-h-12 px-6 py-3 rounded-3xl border border-border bg-background text-foreground text-sm placeholder:text-muted-foreground outline-none focus:border-foreground/30 transition-colors"
              />
              <button
                type="submit"
                disabled={!email.trim() || isCheckingEmail}
                className="w-full min-h-[44px] rounded-full bg-foreground text-background font-bold text-sm transition-all hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed active:scale-[0.98]"
              >
                {isCheckingEmail ? (
        ...
        {step === 'signin' && (
          ...
          >
            ...
            <div className="flex items-center gap-2 px-1">
              <button onClick={goBack} className="text-muted-foreground hover:text-foreground transition-colors p-1 hover:bg-foreground/5 rounded-full">
            ...
            <div className="flex items-center justify-between px-1 pt-1">
              <Link href="/forgot-password" className="text-[13px] text-muted-foreground hover:text-foreground transition-colors underline underline-offset-2">
                Forgot password?
              </Link>
              <button
                onClick={() => { setPassword(''); setStep('signup') }}
        ...
        {step === 'signup' && (
          ...
          >
            ...
            <div className="px-1 pt-1">
              <button
                onClick={() => { setPassword(''); setFullName(''); setStep('signin') }}
                className="text-[13px] text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1"
              >
      ...
      {/* Footer - updated to match Sana style */}
      <motion.p
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.6, delay: 0.4 }}
        className="mt-8 text-[11px] text-muted-foreground/60 leading-[1.6]"
      >
