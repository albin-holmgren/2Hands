...
describe("agent content negotiation", () => {
  ...
  it("maps canonical and trailing-slash page paths to Markdown documents", () => {
    expect(getMarkdownDocument("/")).toContain("# 2hands");
    expect(getMarkdownDocument("/about/")).toContain("# About 2hands");
  ...
  it("publishes specific when-to-use instructions for agents", () => {
    ...
    expect(AGENT_INSTRUCTIONS).toContain("## When to use 2hands");
    expect(AGENT_INSTRUCTIONS).toContain("## How an agent should use 2hands");
  ...
  it("returns cache-safe Markdown responses and omits bodies for HEAD", async () => {
    const response = markdownResponse("# 2hands\n");
    ...
    await expect(response.text()).resolves.toBe("# 2hands\n");

    const headResponse = markdownResponse("# 2hands\n", "HEAD", 404);
