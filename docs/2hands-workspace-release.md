...
- CI passed all 58 application Playwright cases. Browser coverage includes first-run setup, workspace/draft continuity, model controls, approvals and takeover, files, routines, shared conversations, delayed-save isolation, missing-computer recovery, and mobile pane layering/44px close targets. The [application report and 158 screenshots](https://github.com/albin-holmgren/2Hands/actions/runs/33965872457/artifacts/9969468271) and [eight marketing checks and screenshots](https://github.com/albin-holmgren/2Hands/actions/runs/33965872457/artifacts/9969419213) are attached to the draft release PR.
...
- All 180 mobile tests, TypeScript checks and dependency compatibility checks passed; iOS and Android Hermes exports completed. Locally signed iOS Simulator and Android emulator Release builds passed Maestro journeys for sign-in, workspace/model selection, bot creation, scripted chat, computer controls and takeover/release. Separate journeys verified workspace draft isolation and restoration after process death, themed preview failure/retry, and light/dark screens. The Android build completed release lint and APK signature verification; its appearance checks also verified system dark mode and an explicit light override. Native screenshots were inspected separately from web CI screenshots. These journeys use a synthetic backend and do not establish live native computer streaming, provider execution, payment delivery, physical-device behavior or App Store/Play signing. Android screenshots predate a subsequent singular/plural copy correction verified by source checks; the simulators and emulator were shut down after testing.
...
- Dependency remediation passed 57 focused SMTP, MCP and mobile decoder tests, package checks, and a frozen installation. The remaining version-based audit findings and decoder backport are documented in [dependency security](dependency-security.md). A fresh-process development check also compiles and evaluates every translation catalog; production builds alone did not catch the CI formatter-loading failure.
...
The release review additionally hardened computer stream authorization and prepaid reconnect behavior. A real E2B adapter canary passed after fixing a noVNC shell wrapper that retained setup pipes and a template logout script that changed successful setup exit codes. It verified screenshots and files, denial of anonymous traffic, authenticated view access, server-enforced read-only input during takeover, working control input, revocation and rejection of the previous control password, pause/resume file preservation, and confirmed destruction.

The separate full computer journey passed with `vercel-gateway` and `openai/gpt-4.1-mini` on an isolated account and disposable database, with the $1 Free allowance enforced, a three-minute sandbox timeout and a 170-second test deadline. It required a real browser click marker, terminal/file tool results, and restoration of the model-created file onto a different machine after booting through a stale provider reference. A confirmed Stop released the remaining reservation. The final run took 144.34 seconds and settled $0.014202 of model usage plus $0.005981 of computer usage ($0.020183 total), with $0 reserved. Cleanup completed, and a read-only provider inventory confirmed no canary machines remained. Existing provider resources were not changed; inventory alone does not establish legacy network safety. Deterministic recovery/usage/RPC tests passed in an independent 76-test run, and a real PostgreSQL concurrency test verified that expiry cleanup cannot settle a newer computer reservation. These recovery changes add no database migration or subscription-payment semantics.

The real Stripe lifecycle gate passed in a dedicated sandbox, using a fresh local database and a completed hosted Checkout with synthetic card details. It verified Plus/Pro/Ultra allowances, no paid grant for an incomplete subscription, monthly renewal, concurrent duplicate and delayed events, and cancellation through its effective date. All created test resources were cleaned up. A second real TestClock gate verified renewal-scheduled Plus→Pro changes, undo, cancellation and restoration, and a declined renewal without a paid allowance grant. A separate registered public HTTPS webhook received a real paid Checkout and two Stripe redeliveries: all returned 200, the account received its $10 allowance once, and $0.25 of settled spending remained unchanged. The temporary webhook, tunnel and synthetic resources were cleaned up. This proves the route on a temporary test destination; the deployed revision and production signing configuration still require release verification. See [Stripe verification](stripe-release-gate.md). Missing credentials fail preflight; a skipped ordinary unit test is not release evidence.

The deployment's live Stripe catalog was checked against the $20/$60/$200 monthly USD plans. Its existing webhook now includes both invoice payment events as well as Checkout and subscription events, preserving the destination and signing secret. The default customer portal is configured for payment-method updates, invoices, and cancellation at the end of the prepaid period. Portal price changes remain disabled. The application schedules paid plan changes at renewal with the complete monthly price and allowance, preserving the prepaid period. Paid-invoice proof must match the current subscription item, price and period before granting allowance. The web controls show the price and effective date before confirmation and retain cancellation and portal access during payment recovery.
