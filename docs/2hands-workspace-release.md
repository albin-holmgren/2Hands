...
- Native draft/storage tests and typechecking passed; iOS and Android Hermes exports completed. A locally signed iOS Simulator Release build passed a full Maestro smoke journey: sign-in, workspace and model sheets, bot creation, scripted chat, computer viewing, takeover and release. A separate draft journey verified isolation between conversations and restoration after process death. These use a synthetic backend; they do not prove real provider execution, payment delivery, store signing or Android device interaction.
